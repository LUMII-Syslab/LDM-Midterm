...
export default {
    ...
    methods: {
        ...
        getData: function() {
            let self = this;
            axios.get(self.SERVER_URL() + 'training-data/' + self.projectId + "/" + self.currentPage, self.getHeader(self))
        ...
        processData: function(response) {
            ...
            let images = _.map(data.images, function(img) {
                                var path = self.SERVER_URL() + 'get-training-file/' + img.src + "/" + img.project_id;
                                return {_id: self.getId(img), name: img.src, class: img.class, path: path,};
                            });
        ...
        uploadTrainingDataSetClicked: function() {
			...
			form_data.append('zip_file', this.$refs.train_data_fileInputRef.files[0]);
		
            axios.post(this.SERVER_URL() + 'upload-training-data/' + this.projectId, form_data, this.getHeader(this))
    				.then(function(response) {		     
        ...
        download: function() {
            ...
            axios.get(self.SERVER_URL() + 'get-training-set/' + self.projectId, header)
                    .then(function(response) {
        ...
        runChanged: function(event) {
            ...
            this.$router.push({name: 'TrainingData', params: {project_id: this.projectId, run_id: run_id,}});
